...
[lb,cb]=size(L);
%criação do arquivo para B
saida1teste = 'C:\Users\Javier\Desktop\Natal\saidaB.xls'; %adição do caminho
 fid = fopen(saida1teste,'a');texto = ['DADOS ORIGINAIS'];
    texto = char(texto);
 fprintf(fid,texto);
 fclose(fid);

%criacao do arquivo breduc
saida2 = 'C:\Users\Javier\Desktop\Natal\saidaBreduc.xls'; %adição do caminho
 fid = fopen(saida2,'a');texto = ['DADOS reduzidos'];
    texto = char(texto);
 fprintf(fid,texto);
 fclose(fid);
...
for i=1:sizeB
    i;%%%%%%%%%%%%%%%%%%%%%%%%%%%
    B1excel = length(B{i});
     fid = fopen(saida1teste,'a');
     texto2 = [B1excel];
     %texto2 = str2num(cell2mat(B));
       texto2 = char(texto2);
       %texto3 = char(texto2);
    fprintf(fid,texto2);
    fclose(fid);
    ...
    if (size (Bxreduc) ~= 0)%%%%%%%%%%%
    %%%%%%%%% encontrar el maximo que se hace cero 
    %%%%%%%%%%%%%%
    %pause%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %pause
